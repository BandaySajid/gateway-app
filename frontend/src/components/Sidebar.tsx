...
const Sidebar = () => {
	...
	return (
		...
		>
			...
			>
				{isCollapsed ? <MenuIcon className="h-6 w-6" /> : <X className="h-6 w-6" />}
			</Button>

			<div className={cn('fixed h-full bg-neutral-950 w-full', isCollapsed ? 'w-0' : 'w-full md:w-64 min-h-screen')}>
				{!isCollapsed && (
					<nav className="p-4 w-full pt-20">
						<ul className="space-y-2">
								...
								<>
									...
									<li>
										<Link
											onClick={toggleSidebar}
											to="/dashboard"
											className="flex items-center space-x-2 hover:bg-neutral-700 p-2 rounded hover:bg-neutral-900 hover:text-cyan-400 text-neutral-400"
										>
											<span>
												Dashboard{' '}
												<Badge className="text-neutral-50" variant={'outline'}>
													coming soon
												</Badge>
											</span>
										</Link>
							...
							{!ac?.authState.isAuthenticated && <SLink to="/auth" toggleSidebar={toggleSidebar} text="Signin" />}
								...
								<li>
									<AlertDialog>
										<AlertDialogTrigger asChild>
											<Button
												variant={'outline'}
												className="ml-1 flex items-center space-x-2 p-2 rounded text-red-600 border-neutral-800 bg-neutral-950 hover:bg-neutral-900 hover:text-neutral-50"
											>
												<span>Log Out</span>
											</Button>
										</AlertDialogTrigger>
										<AlertDialogContent className="border-neutral-800 bg-neutral-950">
											...
											<AlertDialogFooter>
												<AlertDialogCancel className="text-white border-neutral-800 bg-neutral-950 hover:bg-neutral-800 hover:text-neutral-50">
													Cancel
												</AlertDialogCancel>
												<AlertDialogAction onClick={() => ac.logout()} className="shadow-sm bg-red-900 text-neutral-50 hover:bg-red-900/90">
													Log Out
												</AlertDialogAction>
											</AlertDialogFooter>
										</AlertDialogContent>
